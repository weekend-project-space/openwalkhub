...
async (args)=> {
  ...
  const helper = globalThis.__XhsHelper?.rememberNoteTokens
    ? globalThis.__XhsHelper
    : (globalThis.__XhsHelper = (() => {
    ...
    function getTokenCache() {
      if (!globalThis.__XhsTokenCache) globalThis.__XhsTokenCache = {};
      return globalThis.__XhsTokenCache;
    }
  ...
  if (!userStore?.loggedIn) return { error: "Not logged in"};
  if (!pinia?._s) {
  ...
  XMLHttpRequest.prototype.send = function(body) {
    if (String(this.__url || "").includes("search/notes")) {
      const request = this;
